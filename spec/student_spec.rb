...
describe Student do
  ...
  context 'has instance methods to insert data into db' do 
    ...
    describe '#save' do 
      it 'saves the student to the db' do 
        new_student.save
        expect(DB[:conn].execute("SELECT * FROM students WHERE name = 'Sam'")).to eq([{"grade"=>11, "name"=>"Sam", "id"=>1}])
      end
  ...
  describe '.find_by_name' do 
    it 'executes the SQL to find a row by name' do 
      ...
      expect(Student.find_by_name("Jan")).to eq([{"grade"=>10, "name"=>"Jan", "id"=>1}])
    end
  end
  ...
  describe '.find_by' do 
    it 'executes the SQL to find a row by the attribute passed into the method' do 
      ...
      expect(Student.find_by({name: "Susan"})).to eq([{"grade"=>10, "name"=>"Susan", "id"=>1}])
    end

    it 'accounts for when an attribute value is an integer' do
      Student.new({name: "Susan", grade: 10}).save
      Student.new({name: "Geraldine", grade: 9}).save
      expect(Student.find_by({grade: 10})).to eq([{"grade"=>10, "name"=>"Susan", "id"=>1}])
    end
  end
end
